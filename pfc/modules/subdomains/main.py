...
class SubdomainsModule(Module):
    def execute(self, context):
        ...
        for domain in context.args['domain']:
            ...
            logger.info('Scanning for subdomains of %s ...' % domain)

            for source in sources.sources:
                try:
                    logger.debug('Scraping source %s for subdomains of %s' % (source.NAME, domain))
                    subdomains.update(sanitize_cases(source.scan(self, domain), domain))

                    logger.debug('Found %d new results.' % (len(subdomains) - prev_size))
